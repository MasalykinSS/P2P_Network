...
/*void TalkToServer::listenLoop()
{
    while(true)
    {
        ...
        std::cout<<"i am in loop, ready to listen";
        acceptor_.accept(listener,ec);
        if(ec)
        {
            std::cerr << "accept failed\n";
        }
        boost::asio::read_until(listener,sb,"\0",ec);
        if(ec)
        {
            std::cerr << "read_until failed" << ec << std::endl;
        }
        ...
        sender.connect(ep,ec);
        if(ec)
        {
            std::cerr << "connection error" << ec << std::endl;
        }

        std::string filePath("Files/" + commands[0]);
        std::ifstream ifs(filePath.c_str(), std::ios::binary);
        if(!ifs)
        {
...
void TalkToServer::doLoop()
{
    //std::thread th(&TalkToServer::listenLoop,this);
    //th.detach();
}
